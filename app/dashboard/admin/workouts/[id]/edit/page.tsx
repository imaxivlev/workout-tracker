...
import { adminApi, WorkoutInput, Workout } from '@/lib/api/client';
...
function mapWodExercises(exercises: any[]): WodExerciseForm[] {
  return exercises.map(ex => ({
    ...
    exerciseNameFemale: ex.exerciseFemale?.name ? enToRuName(ex.exerciseFemale.name) : '',
...
function workoutToBlocks(workout: any): BlockItem[] {
  const blocks: BlockItem[] = [];

  for (const sb of workout.skillBlocks) {
    const weightIsPercent = sb.sets.some((s: any) => s.weightIsPercent);
    blocks.push({
      type: 'skill',
      data: {
        exerciseName: enToRuName(sb.exercise.name),
        weightIsPercent,
        sets: sb.sets.map((s: any) => ({
          reps: String(s.reps),
...
export default function AdminEditWorkoutPage() {
  ...
  useEffect(() => {
    adminApi.getWorkoutFull(id)
      ...
      .catch(() => setLoadError('Ошибка загрузки тренировки'))
  ...
  function removeWodExercise(idx: number, exIdx: number) {
    ...
    if (block?.type === 'wod' && block.data.exercises.length <= 1) return;
    updateWodData(idx, b => ({
      ...b,
  ...
  function removeScaledExercise(idx: number, exIdx: number) {
    const block = blocks[idx];
    if (block?.type === 'wod' && block.data.scaledExercises.length <= 1) return;
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      await adminApi.updateWorkoutFull(id, payload);
  ...
  if (loadError) {
    return (
      <div className="container">
        ...
        <button type="button" className="btn-secondary" onClick={() => router.push('/dashboard/admin/workouts')} style={{ marginTop: '1rem' }}>
          ← Назад
        </button>
      </div>
    );
  ...
  return (
    <div className="container">
      <h1 className="page-title">Редактировать тренировку (Админ)</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group" style={{ marginBottom: '1.5rem' }}>
          <label className="checkbox-container" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
            <input type="checkbox" checked={isClubTemplate} onChange={e => setIsClubTemplate(e.target.checked)}
              style={{ width: '18px', height: '18px' }} />
            <span>Шаблон клуба</span>
          </label>
        </div>
              ...
              return (
                <div key={`block-${bi}`} className="added-block skill-block">
                  ...
                  <div className="form-group">
                    ...
                    <select className="form-select sets-selector" value={skill.sets.length}
                      onChange={e => updateSkillSetsCount(bi, parseInt(e.target.value))}>
                      {Array.from({ length: 12 }, (_, i) => i + 1).map(n => (
                        <option key={n} value={n}>{n} {n === 1 ? 'подход' : n < 5 ? 'подхода' : 'подходов'}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <div className="sets-inputs-container sets-container">
                      {skill.sets.map((set, si) => (
                        <input key={si} type="number" value={set.reps}
                          onChange={e => updateSkillSet(bi, si, 'reps', e.target.value)}
                          className="form-input set-input" placeholder={`${si + 1}`} min="1" required />
                      ))}
                    </div>
                  </div>

                  {!shouldHideWeight(skill.exerciseName) && (
                    <div className="form-group">
                      ...
                      <div className="sets-inputs-container sets-container weight-inputs-container">
                        {skill.sets.map((set, si) => (
                          <input key={si} type="number" value={set.weight}
            ...
            return (
              <div key={`block-${bi}`} className="added-block wod-block">
                ...
                <div className="wod-mode-toggles">
                  <div className="wod-toggle-row" onClick={() => {
                ...
                <div className="form-row">
                  <div className="form-group">
                    <label>Тип</label>
                    <select value={wod.wodType} onChange={e => updateWodBlock(bi, { wodType: e.target.value as WodType })} className="form-select">
                  ...
                  {!wod.hasSeparateScaled && (
                    <div className="form-group">
                      <label>Уровень</label>
                      <select value={wod.level} onChange={e => updateWodBlock(bi, { level: e.target.value as WodLevel })} className="form-select">
                ...
                {wod.isLadder && (
                  <div className="form-group ladder-rounds-container">
                    ...
                    <select className="form-select" value={wod.ladderRounds}
                      onChange={e => updateWodBlock(bi, { ladderRounds: parseInt(e.target.value) })}>
                      {Array.from({ length: 12 }, (_, i) => i + 1).map(n => (
                        <option key={n} value={n}>{n} {n === 1 ? 'раунд' : n < 5 ? 'раунда' : 'раундов'}</option>
                      ))}
                    </select>
                  </div>
                )}
                ...
                <div className="form-group">
                  <label>
                    {wod.hasSeparateScaled
                      ? <span style={{ color: 'var(--color-primary)', fontWeight: 600 }}>Rx план</span>
                      : wod.hasGenderSplit
                        ? <><span style={{ color: 'var(--color-primary)', fontWeight: 600 }}>М:</span> Упражнения</>
                        : 'Упражнения'}
                  </label>
                      ...
                      <div key={ei}>
                        <div className={`wod-exercise-row${wod.isLadder ? ' ladder-mode' : ''}${shouldHideWeight(ex.exerciseName) ? ' no-weight' : ''}${wod.hasGenderSplit ? ' has-gender-split' : ''}`}>
                          ...
                          <div className="wod-row-scroller">
                            <ExerciseAutocomplete value={ex.exerciseName}
                              onChange={v => updateWodExercise(bi, ei, 'exerciseName', v)}
                              placeholder="Упражнение" inputClassName="form-input-sm" wrapperClassName="wod-exercise-name" />
                            <div className="wod-fields-scroll">
                              {wod.isLadder ? (
                                <div className="ladder-reps-container" style={{ display: 'flex', gap: '0.5rem' }}>
                                  {Array.from({ length: wod.ladderRounds }, (_, ri) => (
                                    <input key={ri} type="number" value={ex.ladderRepsPerRound[ri] || ''}
                                      onChange={e => updateWodLadderRep(bi, ei, ri, e.target.value)}
                                      className="form-input-sm" placeholder={`R${ri + 1}`} min="1" required />
                                  ))}
                                </div>
                              ) : (
                                <div className="single-reps-container" style={{ display: 'flex' }}>
                                  <input type="number" value={ex.reps}
                                    ...
                                    placeholder={isCardio(ex.exerciseName) ? 'Cal' : 'Повт.'} min="1" required />
                                </div>
                              )}
                              {!shouldHideWeight(ex.exerciseName) && (
                                <input type="number" value={ex.weight}
                                  ...
                                  className="form-input-sm wod-weight" placeholder="Вес, кг" min="0.5" step="0.5" />
                              )}
                            </div>
                          </div>
                          {!wod.hasGenderSplit && (
                        ...
                        {wod.hasGenderSplit && (
                          <div className={`wod-exercise-row-female${wod.isLadder ? ' ladder-mode' : ''}`}>
                            ...
                            <div className="wod-row-scroller">
                              ...
                              <div className="wod-fields-scroll">
                                <div className="single-reps-container" style={{ display: 'flex' }}>
                                  <input type="number" value={ex.repsFemale}
                                    onChange={e => updateWodExercise(bi, ei, 'repsFemale', e.target.value)}
                                    className="form-input-sm" placeholder={isCardio(ex.exerciseName) ? 'Cal' : 'Повт.'} min="1" />
                                </div>
                                {!shouldHideWeight(ex.exerciseName) && (
                                  <input type="number" value={ex.weightFemale}
                  ...
                  <button type="button" onClick={() => addWodExercise(bi)} className="btn-add">+ Добавить упражнение</button>
                </div>
                ...
                {wod.hasSeparateScaled && (
                  <div className="form-group" style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '2px dashed var(--border-color)' }}>
                    ...
                    <button type="button" onClick={() => addScaledExercise(bi)} className="btn-add">+ Добавить упражнение</button>
                  </div>
                )}
                ...
                {wod.wodType !== 'EMOM' && wod.wodType !== 'TABATA' && (
                  <div className="form-group" style={{ paddingTop: '1rem', borderTop: '1px solid var(--border-color)' }}>
                    ...
                    <div className="form-row" style={{ gap: '0.5rem' }}>
                      <select className="form-select" value={wod.wodType === 'AMRAP' ? 'reps' : 'time'}
                        onChange={e => updateWodBlock(bi, { wodType: e.target.value === 'reps' ? 'AMRAP' : 'FOR_TIME' })}>
                        <option value="time">Время</option>
                        <option value="reps">Количество повторений</option>
                      </select>
                      {wod.wodType === 'AMRAP' ? (
                        <input type="number" value={wod.resultTotalReps}
                          onChange={e => updateWodBlock(bi, { resultTotalReps: e.target.value, resultDisplay: e.target.value })}
                          className="form-input result-input" placeholder="420" min="1" required={!isClubTemplate} />
                      ) : (
                        <input type="text" inputMode="numeric" value={wod.resultDisplay}
                          onChange={e => {
                          ...
                          className="form-input result-input" placeholder="ММ:СС" maxLength={5}
        ...
        <div className="form-group" style={{ marginTop: '2rem' }}>
          ...
          <textarea value={comment} onChange={e => setComment(e.target.value)}
            className="form-input" rows={3} placeholder="пример: чудесная тренировка" maxLength={500} />
        </div>

        {error && <div className="form-error">{error}</div>}

        <div className="form-actions">
          <button type="button" onClick={() => router.push('/dashboard/admin/workouts')} className="btn-secondary">
            Отмена
          </button>
