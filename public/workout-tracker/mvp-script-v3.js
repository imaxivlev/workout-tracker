...
function updateAllExerciseRows() {
    ...
    rows.forEach(row => {
        ...
        if (isLadderMode) {
            ...
            // Add/remove many-rounds class based on round count
            if (ladderRounds > 3) {
                row.classList.add('many-rounds');
            } else {
                row.classList.remove('many-rounds');
            }
        } else {
            row.classList.remove('many-rounds');
        }
...
function addWodExerciseRow(button) {
    ...
    if (isLadderMode) {
        ...
        // Add many-rounds class if needed
        if (ladderRounds > 3) {
            newRow.classList.add('many-rounds');
        }
    }
}
...
function changeMonth(delta) {
    // Limit navigation to prevent going to future months (Requirement 3.2)
...
function setupDateInputMask(inputElement) {
    ...
    // Prevent deletion of separators
    inputElement.addEventListener('keydown', (event) => {
        if (event.key === 'Backspace' || event.key === 'Delete') {
            ...
            if (event.key === 'Backspace' && cursorPos > 0 && value[cursorPos - 1] === '.') {
                ...
                // Move cursor before the dot
                inputElement.setSelectionRange(cursorPos - 1, cursorPos - 1);
            } else if (event.key === 'Delete' && cursorPos < value.length && value[cursorPos] === '.') {
                event.preventDefault();
        ...
        if (value) {
            ...
            if (parts.length === 3) {
                ...
                if (!isNaN(day) && !isNaN(month) && !isNaN(year)) {
                    const date = new Date(year, month, day);
                    if (date.getDate() === day && date.getMonth() === month) {
                        // Auto-sort dates (Requirement 2.3)
                        if (datePickerState.startDate && date < datePickerState.startDate) {
                            // Swap dates
...
function selectDate(dateStr) {
    ...
    if (!datePickerState.startDate || (datePickerState.startDate && datePickerState.endDate)) {
        ...
        // Complete range with auto-sort (Requirement 2.3)
...
function getInitialDisplayMonth() {
    ...
    if (datePickerState.endDate) {
        ...
        // No dates selected - show current month as the LAST in the row (Requirement 9.2)
...
function updateNavButtons() {
    ...
    lastShownMonth.setMonth(lastShownMonth.getMonth() + monthsToShow - 1);
    lastShownMonth.setDate(1); // Normalize to first day

    // Disable and hide next button if last shown month is current month or later
